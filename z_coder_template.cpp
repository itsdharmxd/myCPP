...
template <typename TT, int MX, int LG, ll INF>
struct SparseTable //Warning : Change query return value manually if needed. INF is the dummy val
{
    TT st[LG][MX];
    TT initial[MX];
...
struct SegmentTree //by socketnaut. see http://codeforces.com/blog/entry/20528
{
    int S, H;
...
template <int MAXLEN>
struct SuffixAutomaton //check if it works (works only for 'a' - 'z', for general alphabet sets see code from SUBST1
{
    struct state
    {
        int len, link;
...
struct ConvexHull
{
    ...
    ll query(ll x)
    {
        while (int(d.size()) > 1 && (d[0].c - d[1].c <= x * (d[1].m - d[0].m)))
            d.pop_front();
...
struct halfplanes
{
    ...
    void unique()
    {
        ...
        for (i = 1; i < n; i++)
        {
            if (dblcmp(hp[i].angle - hp[i - 1].angle))
                hp[m++] = hp[i];
            else if (dblcmp(hp[m - 1].b.sub(hp[m - 1].a).det(hp[i].a.sub(hp[m - 1].a)) > 0))
                hp[m - 1] = hp[i];
        }
        n = m;
    }
    bool halfplaneinsert()
    {
        ...
        for (i = 2; i < n; i++)
        {
            while (st < ed && dblcmp((hp[i].b.sub(hp[i].a).det(p[ed].sub(hp[i].a)))) < 0)
                ed--;
            while (st < ed && dblcmp((hp[i].b.sub(hp[i].a).det(p[st + 1].sub(hp[i].a)))) < 0)
                st++;
            que[++ed] = i;
        ...
        while (st < ed && dblcmp(hp[que[st]].b.sub(hp[que[st]].a).det(p[ed].sub(hp[que[st]].a))) < 0)
            ed--;
        while (st < ed && dblcmp(hp[que[ed]].b.sub(hp[que[ed]].a).det(p[st + 1].sub(hp[que[ed]].a))) < 0)
            st++;
        if (st + 1 >= ed)
            return false;
        return true;
            ...
            for (int i = 0, relax = false; i < n && relax; i++, relax = false)
            {
                for (int u = 0; u < n; u++)
                {
                    for (int k = 0; k < graph[u].size(); k++)
                    {
                        int eIdx = graph[u][i];
                        int v = e[eIdx].v;
                        ll cap = e[eIdx].cap, w = e[eIdx].cost;
...
struct ConvexHullDynamic : public multiset<Line>
    ...
    for (int b = 1; b <= n / 2; b *= 2)
    {
        ...
        for (int i = 0; i < n; i += (b << 1))
        {
            ...
            for (int j = i; j < i + b; j++)
            {
                ...
                v[j] = t1 + t2;
                v[j] = (v[j] < base ? v[j] : v[j] - base);
                v[j + b] = t1 - t2 + base;
                v[j + b] = (v[j + b] < base ? v[j + b] : v[j + b] - base);
                p = 1LL * p * x % base;
            }
        }
    }
...
int lagrange_function(int x, std::vector<int> &v, int m)
{
    ...
    int ret = 0;
    for (int i = 0; i <= n; i++)
        ret = (ret + 1LL * v[i] * modpow(x - i + m, m - 2, m)) % m;
...
struct edmond
{
    ...
    {
        if (u != v)
            pq[v] = merge(pq[v], &(mem[m++] = node(u, v, w)));
    }
...
vi graph[N];
int U[M], V[M];   //edge list representation of the graph.
bool isbridge[M]; // if i'th edge is a bridge edge or not
int visited[N];
int arr[N], T; //supporting arrays
int cmpno;
queue<int> Q[N];
...
public:
    ...
    void expose()
    {
        for (lct_node *x = this, *y = nullptr; x; y = x, x = x->p)
...
{
    if (vert == stop)
        return; //pathNum is the number of the path to store
    else if (parity == 0)
...
{ //newBlossom() will be called after the paths are evaluated.
    ...
    for (int j = i; j < endPath[0]; j++)
        blossom[path[0][j]] = base;
    for (int j = i + 1; j < endPath[1]; j++)
        blossom[path[1][j]] = base; //Set all nodes to this
    for (int p = 0; p < 2; p++)
    { //new blossom.
